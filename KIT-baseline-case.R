...
dat_inc_poland <- read.csv(paste0(path_hub, "/data-truth/MZ/truth_MZ-Incident Cases_Poland.csv"),
                           colClasses = c(date = "Date"), stringsAsFactors = FALSE)
...
if(!(all(weekdays(all_forecasts$target_end_date) == "Saturday"))){
  ...
  # store:
  write.csv(all_forecasts, file = paste0("forecasts/", forecast_date, "-Poland-KIT-baseline-case.csv"), row.names = FALSE)
}
